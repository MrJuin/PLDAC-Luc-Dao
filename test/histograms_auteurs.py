...
dictio = make_author_database(statements)

name_job, nbr_job = np.unique(dictio['job'], return_counts = True)
...
nbr_fois_job   = np.histogram(nbr_job, bins = max(nbr_job)-1)
...
plt.show()
"""
# Nbr de personne par job
plt.rcdefaults()
fig, ax = plt.subplots(1,1, figsize = (10,100))

y_pos = np.arange(len(name_job)-1)
ax.barh(y_pos, nbr_job[1:], color = 'red')
ax.set_yticks(y_pos)
ax.set_yticklabels(name_job[1:], rotation = 0)
ax.set_title("Distribution des auteurs par métiers")
