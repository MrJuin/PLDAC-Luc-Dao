...
def print_histo(y):    
    ...
    sns.barplot(y =y, x="true",  data=new_dico, orient = 'h', ci = None, \
                label="Pants on fire", color = (1, 0, 0), estimator = np.sum)
    """    
    ...
    """
# Affichage des parties politiques

plt.rcdefaults()
sns.set_theme()
fig, ax = plt.subplots(1,1, figsize = (15,20))
print_histo("party")
ax.set_title("Distribution des messages par partie politique")
ax.set_ylabel("Parties politiques")
...
fig, ax = plt.subplots(1,1, figsize = (15,20))
print_histo("state")
ax.set_title("Distribution des messages par états des états unis")
ax.set_ylabel("états américains")
ax.set_xlabel("Nombre de message")
ax.legend()
plt.show()

# Affichage du nombre de publication
new_dico['pants-fire']    = list(dico['score'].sum(axis = 1))
new_dico['true']          = list(dico['score'][:,[0]].sum(axis = 1))
new_dico['mostly-true']   = list(dico['score'][:,[0,4]].sum(axis = 1))
new_dico['half-true']     = list(dico['score'][:,[0,4, 3]].sum(axis = 1))
